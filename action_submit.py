...
try:
    usernames = os.environ["username"].split("\n")  # 账号
...
def get_yesterday(s: requests.Session):
    data = s.get(
        "https://itsapp.bjut.edu.cn/ncov/api/default/daily?xgh=0&app_id=bjut")
    json_data = data.json()
...
def send_message(key, message, clock_info):
    send_url = "https://sc.ftqq.com/{}.send?text={}&desp={}".format(
        key, message, clock_info)
